...
// Tile component with delayed border animation
interface TileComponentProps {
  tile: GameTile;
  isSelectedByCurrentPlayer: boolean;
  isFlipped: boolean;
  disabled: boolean;
  valueShadow: string;
  onTileClick: (index: number) => void;
...
const TileComponent: React.FC<TileComponentProps> = ({
  ...
  // Show border with 300ms delay after tile is flipped and selected (like Miner)
  const [showBorder, setShowBorder] = useState(false);

  useEffect(() => {
    if (isSelectedByCurrentPlayer && isFlipped) {
  ...
  }, [isSelectedByCurrentPlayer, isFlipped]);

  const handleTileClick = () => {
    if (disabled || tile.selected || tile.revealed) return;
    ...
    >
      ...
      >
        ...
        >
          ...
          {allTilesRevealed && !isSelectedByCurrentPlayer && isFlipped && (
            <div
      ...
      {/* Selected indicator ring - Фиолетовый бордер с задержкой (как в Miner) */}
      {/* NO GLOW - only border, exactly like Miner */}
      {isSelectedByCurrentPlayer && showBorder && (
        <div className="absolute inset-0 rounded-lg border-4 border-purple-500 pointer-events-none z-10 transition-opacity duration-300 opacity-100" />
      )}
    </div>
  );
};
...
export const TileGrid: React.FC<TileGridProps> = ({
  ...
  return (
    <div
      className="grid grid-cols-4 gap-1.5 md:gap-2 p-2 mx-auto"
      style={{
        width: "100%",
      ...
      {tiles.map((tile) => {
        ...
        return (
          <TileComponent
            key={tile.index}
            tile={tile}
            isSelectedByCurrentPlayer={isSelectedByCurrentPlayer}
            isFlipped={isFlipped}
            disabled={disabled}
