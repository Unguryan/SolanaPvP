...
import { MinerTile, MinerGameState, MinerGamePlayer, MinerGameResult } from "@/types/miner";
...
export const MinerGameBoard: React.FC<MinerGameBoardProps> = ({
  ...
  const willWin = useMemo(() => {
    return currentPlayerData?.willWin ?? (isDemoMode ? true : false);
  }, [currentPlayerData, isDemoMode]);
  ...
  const opponentRevealTimersRef = useRef<Map<string, NodeJS.Timeout>>(new Map());
  ...
  const [localGameResult, setLocalGameResult] = useState<MinerGameResult | null>(null);
  ...
  useEffect(() => {
    ...
    if (gameState.status !== "waiting") {
      console.log(`[MinerGameBoard] Status is ${gameState.status}, not initializing (already started or loading)`);
      return;
    }
    
    // Early return checks - must come first
    if (isWaitingForPlayers || isWaitingForScores) {
      console.log(`[MinerGameBoard] Skipping init: isWaitingForPlayers=${isWaitingForPlayers}, isWaitingForScores=${isWaitingForScores}`);
      return;
    }
    
    // Additional protection: check ref (for edge cases)
    if (initializationStartedRef.current) {
      console.log("[MinerGameBoard] Initialization already started (ref check), skipping");
      return;
    }

    if (!isDemoMode && gameState.players.length < maxPlayers) {
      console.log(`[MinerGameBoard] Not enough players: ${gameState.players.length} < ${maxPlayers}, not initializing`);
      return;
    }
    ...
    console.log("[MinerGameBoard] ✅ Setting initializationStartedRef to true - starting initialization");
    ...
    if (currentPlayerData && !currentPlayerResultClickRef.current) {
      const resultClick = determineResultClick(tileCount);
      currentPlayerResultClickRef.current = resultClick;
      console.log(`[MinerGameBoard] Current player ${currentPlayerData.username}: willWin=${currentPlayerData.willWin}, will find result on ${resultClick}-th click`);
    }
    ...
    gameState.players.forEach((p) => {
      const result = p.willWin === true ? "✅ WILL WIN (Alive)" : "❌ WILL LOSE (Bombed)";
      console.log(`  - ${p.username}: willWin=${p.willWin} ${result}, pubkey=${p.pubkey}`);
    });
    ...
    initializationTimeoutRef.current = setTimeout(() => {
      ...
      const initialTiles: MinerTile[] = Array.from({ length: tileCount }, (_, i) => ({
        index: i,
        type: "empty",
        selected: false,
        revealed: false,
        ...
        console.log(`[MinerGameBoard] Setting up timers for ${aiPlayers.length} AI players`);
        aiPlayers.forEach((aiPlayer) => {
          console.log(`  - AI player ${aiPlayer.username}: willWin=${aiPlayer.willWin}`);
          ...
          const timer = setTimeout(() => {
            console.log(`[MinerGameBoard] Revealing result for AI player: ${playerUsername}, willWin=${playerWillWin}`);
            // Use functional update to get latest state
            setGameState((prev) => {
              const player = prev.players.find((p) => p.username === playerUsername);
              console.log(`[MinerGameBoard] Player before update:`, player ? { username: player.username, willWin: player.willWin, isScoreRevealed: player.isScoreRevealed } : 'NOT FOUND');
              
              // Check if already revealed to prevent duplicate updates
              if (player?.isScoreRevealed) {
                console.log(`[MinerGameBoard] Player ${playerUsername} already revealed, skipping`);
                return prev;
              }
              ...
              const newPlayers = prev.players.map((p) => {
                if (p.username === playerUsername) {
                  ...
                  console.log(`[MinerGameBoard] Player after update:`, { username: updatedPlayer.username, willWin: updatedPlayer.willWin, isScoreRevealed: updatedPlayer.isScoreRevealed, isAlive: updatedPlayer.isAlive });
                  return updatedPlayer;
                }
                return p;
              });

              console.log(`[MinerGameBoard] State update - new players:`, newPlayers.map(p => ({ username: p.username, isScoreRevealed: p.isScoreRevealed, willWin: p.willWin })));
    ...
    return () => {
      ...
      if (initializationCompletedRef.current || initializationStartedRef.current) {
        // Initialization started - don't cleanup, let it complete
        console.log(`[MinerGameBoard] Cleanup: initialization started (refs: started=${initializationStartedRef.current}, completed=${initializationCompletedRef.current}), KEEPING timeout`);
      ...
      if (initializationTimeoutRef.current) {
        console.log("[MinerGameBoard] Cleanup: initialization not started, clearing timeout");
        clearTimeout(initializationTimeoutRef.current);
        initializationTimeoutRef.current = null;
  ...
  const handleTileClickRef = useRef<((index: number, isAutoOpen?: boolean) => void) | null>(null);
  ...
  const handleTileClick = useCallback(
    (index: number, isAutoOpen: boolean = false) => {
      ...
      if (resultClick === null) {
        console.warn(`[MinerGameBoard] ⚠️ resultClick is NULL! currentPlayerResultClickRef.current=${currentPlayerResultClickRef.current}`);
      }
      console.log(`[MinerGameBoard] Tile clicked: index=${index}, clickNumber=${openedCount}, resultClick=${resultClick}, tileType=${tileType}, shouldEndGame=${shouldEndGame}`);
      ...
      if (shouldEndGame) {
        console.log(`[MinerGameBoard] 🎉 Player found result on ${openedCount}-th click! Type: ${tileType}`);
        ...
        if (autoOpenCleanupRef.current) {
          console.log(`[MinerGameBoard] Stopping auto-open - player found result manually`);
        ...
        if (timeRemaining <= 0) {
          console.log(`[MinerGameBoard] Timer already at 0, changing status to revealing immediately`);
          setGameState((prev) => ({
            ...prev,
            status: "revealing",
          }));
          ...
          console.log(`[MinerGameBoard] Timer still running (${timeRemaining}s), will change status when timer reaches 0`);
        }
  ...
  useEffect(() => {
    ...
    if (gameState.status !== "playing") {
      ...
      if (timerIntervalRef.current) {
        console.log(`[MinerGameBoard] Status is ${gameState.status}, clearing timer`);
    ...
    if (timerIntervalRef.current) {
      console.log(`[MinerGameBoard] Timer interval already exists (ID: ${timerIntervalRef.current}), skipping recreation`);
      return;
    }

    console.log(`[MinerGameBoard] Starting timer, status=${gameState.status}, initial timeRemaining=${timeRemaining}`);
    ...
    console.log(`[MinerGameBoard] ✅ Timer interval created (ID: ${intervalId}), will tick in 1 second`);

    // Cleanup function - CRITICAL for StrictMode!
    return () => {
      const intervalCreatedHere = timerCreatedInThisCallRef.current;
      const currentInterval = timerIntervalRef.current;
      console.log(`[MinerGameBoard] Cleanup timer - created here: ${intervalCreatedHere}, current: ${currentInterval}`);
      ...
      if (intervalCreatedHere) {
        console.log(`[MinerGameBoard] Clearing interval ${intervalCreatedHere} (created in this call)`);
  ...
  useEffect(() => {
    if (timeRemaining <= 0 && gameState.status === "playing") {
      console.log(`[MinerGameBoard] ⏰ Timer reached 0, status=${gameState.status}`);
      
      // CRITICAL: Check if auto-open already started to prevent double execution in StrictMode
      if (autoOpenStartedRef.current) {
        console.log(`[MinerGameBoard] Auto-open already started, skipping (prevent double in StrictMode)`);
        return;
      }
      ...
      if (playerGameEnded) {
        ...
        console.log(`[MinerGameBoard] Player already found result, changing status to revealing`);
        setGameState((prev) => ({
          ...prev,
          status: "revealing",
        }));
        ...
        console.log(`[MinerGameBoard] ⏰ Timer reached 0, starting auto-open for player: ${currentPlayer}`);
        console.log(`[MinerGameBoard] ⏰ Status will remain "playing" until result is found`);
        ...
        if (currentPlayerData && currentPlayerResultClickRef.current !== null) {
          ...
          if (!handleTileClickRef.current) {
            console.warn(`[MinerGameBoard] handleTileClickRef not set yet, waiting...`);
            // Retry after a short delay to ensure ref is set
            setTimeout(() => {
              if (handleTileClickRef.current && timeRemaining <= 0 && gameState.status === "playing") {
                ...
                const cleanup = autoOpenTiles(
                  ...
                  (tileIndex, clickNumber) => {
                    console.log(`[MinerGameBoard] Auto-opening tile: index=${tileIndex}, clickNumber=${clickNumber}`);
                    if (handleTileClickRef.current) {
                      handleTileClickRef.current(tileIndex, true);
                    } else {
                      console.error(`[MinerGameBoard] handleTileClickRef still not set during auto-open!`);
                    }
                  },
                  () => {
                    console.log(`[MinerGameBoard] Auto-open completed, waiting before changing status to revealing`);
                    setTimeout(() => {
                      setGameState((prev) => {
          ...
          console.log(`[MinerGameBoard] Starting auto-open: resultClick=${resultClick}, currentClickCount=${currentClickCount}, playerWillWin=${playerWillWin}`);
          const cleanup = autoOpenTiles(
            minerTiles,
            resultClick,
            currentClickCount,
            ...
            (tileIndex, clickNumber) => {
              ...
              if (playerGameEnded) {
                console.log(`[MinerGameBoard] Player already found result, stopping auto-open`);
                if (autoOpenCleanupRef.current) {
                  autoOpenCleanupRef.current();
                  autoOpenCleanupRef.current = null;
                  autoOpenStartedRef.current = false;
              ...
              if (handleTileClickRef.current) {
                ...
                console.error(`[MinerGameBoard] handleTileClickRef not set during auto-open! This should not happen.`);
              }
            },
            () => {
          ...
          console.log(`[MinerGameBoard] Auto-open fallback: missing currentPlayerData or resultClick`);
  ...
  }, [timeRemaining, gameState.status, playerGameEnded, currentPlayer, currentPlayerPubkey, gameState.players, minerTiles, getPrizeBombCount]);

  // Reset all refs and cleanup on unmount (when gameKey changes or component unmounts)
  useEffect(() => {
    return () => {
      console.log(`[MinerGameBoard] Component unmounting - status: ${gameState.status}`);
  ...
  useEffect(() => {
    ...
    console.log("[MinerGameBoard] Processing game completion, status=revealing");
    ...
    console.log("[MinerGameBoard] Players:", gameState.players.map(p => ({ username: p.username, willWin: p.willWin, isAlive: p.isAlive, isScoreRevealed: p.isScoreRevealed })));
    ...
    console.log("[MinerGameBoard] Result to pass gameType:", resultToPass.gameType);
    console.log("[MinerGameBoard] Result to pass playerResults:", resultToPass.playerResults);
    console.log("[MinerGameBoard] Result to pass keys:", Object.keys(resultToPass));

    setTimeout(() => {
      if (onGameCompleteRef.current) {
        console.log("[MinerGameBoard] Calling onGameComplete with result:", resultToPass);
  ...
  const shouldHideTeamScores = useMemo(() => {
    ...
    const hasAnyRevealed = gameState.players.some((p) => p.isScoreRevealed === true);
  ...
  if (isWaitingForPlayers) {
    return (
      <WaitingLobby
        ...
        onCancel={() => setGameState((prev) => ({ ...prev, status: "finished" }))}
      />
    );
  }
  ...
  if (isWaitingForScores) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-txt-muted">Preparing your match with epic rewards!</p>
      </div>
    );
  }
        ...
        {currentPlayer !== "" || isDemoMode ? (
          ...
          <div className="text-center py-12">
            <div className="glass-card p-8 max-w-md mx-auto">
              <h3 className="text-2xl font-bold text-sol-purple mb-4">👁️ Spectator Mode</h3>
              <p className="text-txt-muted">
                You are watching this match as a spectator. The game board is hidden, but you can
                see all player scores and team progress above.
              </p>
            </div>
          </div>
        )}
